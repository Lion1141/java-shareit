...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemDtoOut> findAll(Long userId, Integer from, Integer size) {
        ...
        if (from == null || from < 0) {
            throw new IllegalArgumentException("From index must be non-negative.");
        }
        ...
        Map<Long, List<BookingDtoOut>> bookings = bookingRepository.findAllByItemInAndStatusOrderByStartAsc(itemList, BookingStatus.APPROVED)
                .stream()
                .map(BookingMapper::toBookingOut)
        ...
        return itemList.stream()
                .map(item -> ItemMapper.toItemDtoOut(
                        item,
                        getLastBooking(bookings.get(item.getId()), now),
                        comments.get(item.getId()),
                        getNextBooking(bookings.get(item.getId()), now)
                ))
                .collect(toList());
    }
